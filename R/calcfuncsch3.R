...
problistch3[[1]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_tp_x}$
{
  tpx <- n.tpx(SULT, age, tt) 
  s.prob <- paste0(s.tpx(age, tt))
  s.ans <- paste0(s.tpx(age, tt),"=", s.frac(s.ellx(age+tt), s.ellx(age)),"=", s.frac(SULT[age+tt,2], SULT[age,2]), "=",tpx)
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=tpx))  
}

problistch3[[2]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_tq_x}$
{
  tqx <- n.tqx(SULT, age, tt) 
  s.prob <- paste0(s.tqx(age, tt))
  s.ans <- paste0(s.tqx(age, tt),"=", s.frac(s.minus(s.ellx(age),s.ellx(age+tt)),s.ellx(age)), "=", s.frac(s.minus( SULT[age,2], SULT[age+tt,2]), SULT[age,2]), "=",tqx)
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=tqx))  
}

problistch3[[3]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_k|_tq_x}$
{
  ktqx <- n.ktqx(SULT, age, kk, tt)
  s.prob <- paste0(s.ktqx(age, kk, tt))
  s.ans <- paste0(s.ktqx(age, kk, tt),"=", s.frac(s.minus(s.ellx(age+kk),s.ellx(age+kk+tt)),s.ellx(age)), "=", s.frac(s.minus( SULT[age+kk,2], SULT[age+kk+tt,2]), SULT[age,2]), "=",ktqx)
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=ktqx))  
}

problistch3[[4]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_{t+s}p_{x+r}},$ ($s, t$ and/or $r$ may be 0), UDD / CF
{
  ...
  if (FAA == "UDD")
  {
    ...
    tpx <- round(num/den, 5) 
    s.numans1 <- if(abs(floor(endage) - endage) <0.01) s.ellx(floor(endage)) else s.plus(paste0("(1-",ffe,")",s.ellx(floor(endage))), paste0("(",ffe,")",s.ellx(ceiling(endage))))
    ...
    s.ans <- paste0(s.tpx(age + rr, tt + ss),"=", s.frac(s.ellx(endage), s.ellx(startage)),"\\overset{UDD}{=}", s.ans1, "=", s.frac(num, den), "=", tpx)
  } else
  {
    ...
    s.ans <- paste0(s.tpx(age + rr, tt + ss),"=", s.ans1.fac1, s.ans1.fac2, s.ans1.fac3,"\\overset{CF}{=}", s.ans2.fac1, s.ans2.fac2, s.ans2.fac3, "=", s.ans3.fac1, s.ans3.fac2, s.ans3.fac3, "=",tpx)     
  }
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=tpx))  
}

problistch3[[5]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_{t+s}q_{x+r}},$ ($s, t$ and/or $r$ may be 0), UDD / CF
{
  tpx <- do.call(problistch3[[4]], as.list(environment()))
  s.prob <- paste0(s.tqx(age + rr, tt + ss), "\\text{  under  }", FAA)
  tqx <- round(1 - tpx$num.ans, 5)
  s.ans <- paste0(s.tqx(age + rr, tt + ss), " = 1- ",s.tpx(age + rr, tt + ss), " \\overset{", FAA, "}{=} 1- ", tpx$num.ans, "=", tqx, ", \\text{ where }$  \n\\hspace*{0.2in}$\\displaystyle", tpx$s.ans)
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=tqx))  
}

problistch3[[6]] <- function(SULT, age, tt, kk, rr, ss, ww, FAA) # ${_{k+s}|_{t+w}q_{x+r}},$ ($k, s, t, w$ and/or $r$ may be 0), UDD / CF
{
  ...
  if (FAA == "UDD")
  {
    ...
    s.numans1a <- if(abs(floor(midage) - midage) <0.01) s.ellx(floor(midage)) else s.plus(paste0("(1-",ffm,")",s.ellx(floor(midage))), paste0("(",ffm,")",s.ellx(ceiling(midage))))
    s.numans1b <- if(abs(floor(endage) - endage) <0.01) s.ellx(floor(endage)) else s.plus(paste0("(1-",ffe,")",s.ellx(floor(endage))), paste0("(",ffe,")",s.ellx(ceiling(endage))))
    s.denans1 <- if(rr == 0) s.ellx(floor(startage)) else s.plus(paste0("(1-",ffs,")",s.ellx(floor(startage))), paste0("(",ffs,")",s.ellx(ceiling(startage))))
    s.ans1 <- paste0(s.frac(s.minus(paste0("[",s.numans1a,"]"), paste0("[",s.numans1b,"]")), s.denans1)) 
    s.ans <- paste0(s.ktqx(age + rr, kk + ss, tt + ww)," = ", s.frac(s.minus(s.ellx(midage), s.ellx(endage)), s.ellx(startage))," \\overset{UDD}{=} ", s.ans1, "=", s.frac(num, den), "=", ktqx)
  } else
  {
    tpx <- do.call(problistch3[[4]], list(SULT=SULT, age=age, tt=kk, kk=tt, rr=rr, ss=ss, ww=ww, FAA=FAA))
    tqxk <- do.call(problistch3[[5]], list(SULT=SULT, age=floor(midage), tt=floor(endage-midage), kk=NULL, rr=midage-floor(midage), ss=(endage-midage)-floor(endage-midage), ww=NULL, FAA=FAA))
    ktqx <- round(tpx$num.ans*tqxk$num.ans, 5)
    s.ans <- paste0(s.ktqx(age + rr, kk + ss, tt + ww),"=", "\\left(", s.tpx(age+rr, kk+ss), "\\right)", "\\left(", s.tqx(age+rr+kk+ss, tt+ww), "\\right) \\overset{CF}{=} ", "(", round(tpx$num.ans, 5), ")(", round(tqxk$num.ans, 5), ") = ", ktqx,", \\text{ where }$  \n\\hspace*{0.2in}$\\displaystyle", tpx$s.ans, ", \\text{ and }$  \n\\hspace*{0.2in}$\\displaystyle", tqxk$s.ans)     
  }
  return(list(s.prob=s.prob, s.ans=s.ans, num.ans=ktqx))  
}
